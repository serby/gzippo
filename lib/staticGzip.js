  ...
  return function staticGzip(req, res, next){
		...
		fs.stat(filename, function(err, stat) {
			...
			cacheStore.get(gzipName, function(err, cacheObj) {
				if (err) {
					return gzipAndSend(filename, gzipName, stat.mtime);
				}

				if (req.headers['if-modified-since'] &&	+cacheObj.mtime <= new Date(req.headers['if-modified-since']).getTime()) {
					setHeaders(cacheObj);
				...
				if ((cacheObj.mtime < stat.mtime) || ((cacheObj.ctime + maxAge) < Date.now())) {
					return gzipAndSend(filename, gzipName, stat.mtime);
				} else {
					return sendGzipped(cacheObj);
				}
			});
		});
	};
